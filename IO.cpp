...
void CIO::process()
{
    ...
    if (m_rxBuffer.size() >= RX_BLOCK_SIZE) {
        ...
        for (uint16_t i = 0U; i < RX_BLOCK_SIZE; i++) {
            ...
            int16_t res1 = sample - m_rxDCOffset;
            //int32_t res2 = res1 * m_rxLevel;
            samples[i] = std::min(std::max(res1 * m_rxLevel, INT16_MIN), INT16_MAX);
...
//            }
            ...
            if (m_dmrEnable || m_ysfEnable) {
                int16_t RRCVals[RX_BLOCK_SIZE];
                std::copy(samples, samples + RX_BLOCK_SIZE - 1, RRCVals);
...
//            }
            ...
            if (m_dmrEnable) {
                int16_t DMRVals[RX_BLOCK_SIZE];
                std::copy(samples, samples + RX_BLOCK_SIZE - 1, DMRVals);
...
void CIO::write(MMDVM_STATE mode, float* symbols, uint16_t length, const uint8_t* control)
{
    ...
    //    int16_t res2 = std::min(std::max(samples[i] * txLevel * m_rxLevel, INT16_MIN), INT16_MAX); //q15_t(__SSAT((res1 >> 15), 16));
    ...
    //    if (control == NULL)
    //        m_txBuffer.push(std::make_pair(res3, MARK_NONE));
    //    else
    //        m_txBuffer.push(std::make_pair(res3, control[i]));
    //}
}
...
void CIO::setParameters(bool rxInvert, bool txInvert, bool pttInvert, uint8_t rxLevel, uint8_t cwIdTXLevel, uint8_t dstarTXLevel, uint8_t dmrTXLevel, uint8_t ysfTXLevel, uint8_t p25TXLevel, uint8_t nxdnTXLevel, uint8_t pocsagTXLevel, int16_t txDCOffset, int16_t rxDCOffset)
{
    ...
    m_rxLevel = int16_t(rxLevel * 128);
    m_cwIdTXLevel = int16_t(cwIdTXLevel * 128);
    m_dstarTXLevel = int16_t(dstarTXLevel * 128);
    m_dmrTXLevel = int16_t(dmrTXLevel * 128);
    m_ysfTXLevel = int16_t(ysfTXLevel * 128);
    m_p25TXLevel = int16_t(p25TXLevel * 128);
    m_nxdnTXLevel = int16_t(nxdnTXLevel * 128);
    m_pocsagTXLevel = int16_t(pocsagTXLevel * 128);
...
{
    return false;// m_txBuffer.hasOverflowed();
}

bool CIO::hasRXOverflow()
{
    return false;// m_rxBuffer.hasOverflowed();
}
